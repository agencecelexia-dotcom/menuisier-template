  ...
  recentSubmissions: [
    {
      ...
      message:
        "Bonjour, nous souhaitons rénover entièrement notre cuisine pour la transformer en cuisine ouverte sur le séjour. La pièce fait environ 15m².",
    ...
    },
    {
      ...
      message:
        "Bonjour Mickaël, je fais construire une mezzanine dans mon salon et j'aurais besoin d'un escalier hélicoïdal en bois.",
    ...
    },
    {
      ...
      message:
        "Bonjour, nous venons d'emménager et la chambre parentale dispose d'un grand renfoncement parfait pour un dressing sur mesure.",
    ...
    },
    {
      ...
      message:
        "Je souhaite faire réaliser une grande bibliothèque murale sur tout un pan de mur de mon salon (4m50 de large, 2m60 de haut).",
    ...
    },
    {
      ...
      message:
        "Bonjour, nous avons acheté une longère bretonne et les menuiseries intérieures sont en mauvais état.",
...
function getStatusColor(status: string) {
  return (
    STATUS_OPTIONS.find((s) => s.label === status)?.color ??
...
export default function DashboardTab() {
  ...
  const [submissions, setSubmissions] = useState(
    FAKE_STATS.recentSubmissions
  );
  const [statusDropdownId, setStatusDropdownId] = useState<number | null>(
    null
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl p-6 shadow-premium border border-border/50">
          ...
          <p className="text-3xl font-bold text-primary">
            {FAKE_STATS.uniqueVisitors.toLocaleString("fr-FR")}
          </p>
                  ...
                  <td className="py-3 pr-4 relative">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setStatusDropdownId(
                          statusDropdownId === s.id ? null : s.id
                        );
                      }}
                    ...
                    {statusDropdownId === s.id && (
                      ...
                      >
                        {STATUS_OPTIONS.map((opt) => (
                          ...
                          >
                            <span
                              className={`inline-block w-2 h-2 rounded-full ${opt.color.split(" ")[0].replace("50", "500")}`}
                            />
                            {opt.label}
                          </button>
                        ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {STATUS_OPTIONS.map((opt) => (
                    <button
                      key={opt.label}
                      onClick={() =>
                        updateStatus(selectedSubmission.id, opt.label)
                      }
